...
server.on("connection", (ws: WebSocket) => {
  ...
  logWithColor(
    `Client connected and authenticated with id ${connectionId}`,
    COLORS.YELLOW,
  );
});
...
function handleCloseConnection(
  ...
  return () => {
    handler.unsubscribeClient(wsWithId); // Remove all client listeners
...
export function sendMessage(
  ...
  logMessage(
    JSON.stringify(dataResponseDTO, null, 2),
    LogMessageType.SENT,
    ws.id,
  );
}
